...
class PatientsList(models.Model):
    ...
    address = fields.Selection(
        string="Address", selection=[("own", "patient"), ("relative", "Relative")]
    )
    country = fields.Many2one("res.country", string="Country")
    state = fields.Many2one("res.country.state", string="State")
    email = fields.Char(string="E-mail")
    date = fields.Date(
        default=lambda self: fields.Date.today(), string="Registration Date"
    )
    ...
    medical_history_ids = fields.One2many(
        "medical.history", "patient_medical_id", string="Medical History"
    )
